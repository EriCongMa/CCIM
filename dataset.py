...
class Batch_Balanced_Dataset_3(object):
    ...
    def __init__(self, opt):
        ...
        _AlignCollate = AlignCollate_3(imgH=opt.imgH, imgW=opt.imgW, keep_ratio_with_pad=opt.PAD)
        self.data_loader_list = []
        self.dataloader_iter_list = []
        batch_size_list = []
        Total_batch_size = 0
        for selected_d, batch_ratio_d in zip(opt.select_data, opt.batch_ratio):
            ...
            print(dashed_line)
            log.write(dashed_line + '\n')
            _dataset, _dataset_log = hierarchical_dataset_3(root=opt.train_data, opt=opt, select_data=[selected_d])
            total_number_dataset = len(_dataset)
            log.write(_dataset_log)
            ...
            print('*' * 50)
            print('After subset operation, len of _dataset: {}'.format(len(_dataset)))
            for idx in range(5):
                print('Sample No.{} of _dataset: {}'.format(idx, _dataset[idx]))
            ...
            _data_loader = torch.utils.data.DataLoader(
                ...
                num_workers=int(opt.workers),
                collate_fn=_AlignCollate, pin_memory=True)
            # print('len of _data_loader: {}'.format(len(_data_loader)))
    ...
    def get_batch(self):
        ...
        for i, data_loader_iter in enumerate(self.dataloader_iter_list):
            try:
                image, src_text, tgt_text = data_loader_iter.next()
                balanced_batch_images.append(image)
                balanced_batch_src_texts += src_text
                balanced_batch_tgt_texts += tgt_text
            except StopIteration:
                self.dataloader_iter_list[i] = iter(self.data_loader_list[i])
                image, src_text, tgt_text = self.dataloader_iter_list[i].next()
                balanced_batch_images.append(image)
                balanced_batch_src_texts += src_text
                balanced_batch_tgt_texts += tgt_text
        ...
        return balanced_batch_images, balanced_batch_src_texts, balanced_batch_tgt_texts

def hierarchical_dataset_3(root, opt, select_data='/'):
    dataset_list = []
    ...
    for dirpath, dirnames, filenames in os.walk(root+'/'):
        if not dirnames:
            ...
            if select_flag:
                dataset = LmdbDataset_3(dirpath, opt)
...
class AlignCollate_3(object):
    ...
    def __call__(self, batch):
        ...
        return image_tensors, src_labels, tgt_labels
